...
- (void)adjustToContainAnnotationView:(MKAnnotationView *)annotationView{
    // I just like this assert here
    NSParameterAssert(annotationView);
    
    CGRect annotationViewWithCalloutViewFrame = CGRectMake(0, 0, 290, 230);
    annotationViewWithCalloutViewFrame.size.width += 290 ;
    annotationViewWithCalloutViewFrame.size.height += 230 ;
    
    CGRect annotationRect = [self convertRect:annotationViewWithCalloutViewFrame toView:self];
    ...
    [self setRegion:[self regionThatFits:adjustedRegion] animated:YES];
}
